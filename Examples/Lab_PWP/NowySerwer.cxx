...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  //TODO: Get number of points, iterate with loop
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    {
    int nElements = pointMsg->GetNumberOfPointElement();
    igtl::PointMessage::Pointer newPointMsg;
    newPointMsg = igtl::PointMessage::New();
...
// new code
      newPointMsg->SetDeviceName("NowySerwer");
      ...
      // Change position sign to negative
      for (int j = 0; j < (sizeof(pos)/sizeof(*pos)); j++)
      {
          pos[j] = -pos[j];
      }
      ...
      newPointMsg->AddPointElement(point0);
	  newPointMsg->Pack();
    ...
    std::cerr << "Sending point message..." << std::endl;
    socket->Send(newPointMsg->GetPackPointer(), newPointMsg->GetPackSize());
